...
def normalize_target(raw: str) -> str:
    """Normalize a user-supplied target to a bare domain / IPv4 / CIDR.

    Raises ValueError when the target is not an accepted format.  Shared by
    the ScanRequest validator and by scope enforcement so both validate the
    same accepted shapes.
    """
    v = raw.strip().lower()
    if not v:
